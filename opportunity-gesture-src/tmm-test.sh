...
HVite -A -D -T 1 -w wdnet_single -H model/hmm$((train_steps))/all -i hvite.mlf -S test-data/classifylist.txt def/dict.txt hmmlist.txt > reco.mlf
#awk 'NR%3==0' reco.mlf | awk '{print $3}' > $subject_prefix$s-results-simple.txt
awk '/mfcc$/{nr[NR+1]}; NR in nr' reco.mlf | awk '{print $1}' > results-tmm.txt
sed -n 's/^.*_act_\([0-9]*\).*$/\1/p' test-data/testlist.txt | grep --color=never -o '[0-9]\+' | awk '{print "Activity"$0}' > original-tmm.txt
